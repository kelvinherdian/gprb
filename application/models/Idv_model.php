...
class Idv_model extends CI_Model {
	...
	public function listing_idv($id_idv = FALSE) {
	if ($id_idv === FALSE)	{
		$query = $this->db->query('SELECT * FROM idv ORDER BY id_idv DESC');
		return $query->result_array();
	}
	$query = $this->db->get_where('idv', array('id_idv' => $id_idv));
	return $query->row_array();
	}
	
	// Add new User
	public function tambah($data) {
		return $this->db->insert('idv', $data);
	}
	...
	public function delete($data) {
		$this->db->where('id_idv', $data['id_idv']);
		return $this->db->delete('idv', $data);
	}
	
	// Listing data
	public function get_idv($limit, $start) {
		$this->db->select('*');
		$this->db->from('idv');
